...
def sign_up():
    if request.method == 'POST':
        ...
        if user:
            ...
            new_user = Client(email=email, first_name=first_name, last_name=last_name, age=age, password=generate_password_hash(password1, method='sha256'), gender=gender, account_type=account_type)
            ...
            new_user = Nutritionist(email=email, first_name=first_name, last_name=last_name, age=age, password=generate_password_hash(password1, method='sha256'), gender=gender, account_type=account_type)
            db.session.add(new_user)
            db.session.commit()
            login_user(new_user, remember=True)
            flash('Account created!', category='success')
